  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        ...
        if (response.ok) navigate('/beranda');
        else setErrors({ form: 'Login gagal. Periksa kredensial Anda.' });
      } catch {
      ...
      <main className="w-full max-w-md px-gutter flex-1 flex flex-col py-xl">
        ...
        <section className="space-y-lg bento-card p-lg rounded-lg">
          <div className="space-y-xs">
            ...
            <input
              className={`w-full px-md py-3 bg-surface rounded-lg border border-border text-body-base focus:ring-0 focus:border-primary transition-all duration-200 ${errors.email ? 'border-error' : ''}`}
              id="email"
            ...
            />
            {errors.email && <p className="text-error text-body-sm ml-1">{errors.email}</p>}
          </div>
          <div className="space-y-xs">
            ...
            <div className="relative group">
              <input
                className={`w-full px-md py-3 bg-surface rounded-lg border border-border text-body-base focus:ring-0 focus:border-primary transition-all duration-200 pr-12 ${errors.password ? 'border-error' : ''}`}
                id="password"
                placeholder="••••••••"
            ...
            {errors.password && <p className="text-error text-body-sm ml-1">{errors.password}</p>}
            <div className="flex justify-end">
              <a className="text-label-caps font-label-caps text-primary hover:underline transition-all" href="#">Lupa kata sandi?</a>
            </div>
          </div>
        </section>
        {errors.form && <p className="text-error text-body-sm text-center mt-md">{errors.form}</p>}
        ...
        <footer className="mt-auto py-lg text-center">
          <p className="text-body-base text-tx-secondary">
            Belum punya akun? <a className="text-primary font-bold hover:underline ml-1" href="#">Daftar</a>
          </p>
        </footer>
      </main>
    </>
